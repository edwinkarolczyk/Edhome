...
> **Bieżący stan 0.6.0, 24.09.2026:** opublikowana Beta **0.6.0.16 / versionCode 81 / SQLite v33**. Zwijanie kart pojazdów odbywa się przez tapnięcie nagłówka i jest pamiętane per pojazd. W PayCheck wdrożono opcjonalny nasłuch powiadomień wyłącznie od wskazanych aplikacji bankowych, który tworzy lokalne podpowiedzi; **nie księguje automatycznie, nie dowodzi rozliczenia bankowego i wymaga odbioru na rzeczywistym telefonie**. Dotychczasowe testy telefonu: 6/7 OK w 0.6.0.10, **import wyciągu bankowego NIE DZIAŁA według Edwina i wraca na końcową stabilizację 0.6.0**. Załączniki PDF/zdjęcia do dokumentów pojazdów **odłożone decyzją Edwina**, pozostaje rejestr metadanych dokumentu. Po testach powiadomień należy ustabilizować całość i wrócić do importu; nie oznaczać 0.6.0 jako zamkniętej przed odbiorem. Nie rozpoczynać 0.7.0 i nie dotykać `main`.
