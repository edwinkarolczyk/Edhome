...
paycheck31 = statements(section(paycheck_store, "static void create(SQLiteDatabase db)", "static String add(").replace("db.execSQL(", "database.execSQL("))
paycheck20 = [sql.replace(", status TEXT NOT NULL DEFAULT 'confirmed' CHECK(status IN ('pending','confirmed'))", "").replace(", confirmation_source TEXT NOT NULL DEFAULT 'legacy' CHECK(confirmation_source IN ('none','legacy','manual'))", "").replace(", confirmed_at INTEGER", "") for sql in paycheck31]
step30 = statements(section(upgrade, "if (oldVersion >= 20 && oldVersion < 30)", "if (oldVersion < 31)"))
step31 = statements(section(upgrade, "if (oldVersion < 31)", "private static void addPlaceSiblingIndex"))
...
assert version == backup_version == 31, "Database version and backup format differ"

fresh = sqlite3.connect(":memory:")
execute(fresh, create + audit + history + rotations + places + sibling_index + members + shifts + shopping + timers + pantry14 + pantry15 + pantry17 + receipts18 + storage19 + paycheck31 + goals21 + prices22 + vehicles28 + tyres25 + policies27 + costs29)
expected = schema(fresh)
assert len(expected) == 29 and len(costs29) == 2 and len(step30) == 1 and len(step31) == 3 and len(vehicles24) == 3 and len(tyres25) == 3 and len(policies27) == 3 and len(step27) == 1 and len(step28) == 2 and len(step23) == 3 and len(prices22) == 2 and len(goals21) == 3 and len(paycheck20) == 1 and len(storage19) == 3 and len(receipts18) == 1 and len(pantry14) == 4 and len(pantry15) == 1 and len(step16) == 1 and len(pantry17) == 1 and len(legacy17) == 1, "Unexpected number of tables"
...
existing23.close()
print("SQLite migrations v1–v30→v31: PASS; reminders, policies, tyres, vehicles, shopping, pantry, backup: PASS")
