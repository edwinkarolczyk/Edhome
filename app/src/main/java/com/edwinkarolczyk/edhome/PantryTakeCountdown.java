...
final class PantryTakeCountdown {
    ...
    /** A DIFFERENT code always replaces the pending code, resetting its timer. */
    boolean observe(String barcode, long now) {
        if (barcode == null || barcode.isEmpty()
                || barcode.equals(pending)
                || (pending == null && barcode.equals(blocked))) return false;
        pending = barcode;
